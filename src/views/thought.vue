  ...
  <v-container class="pa-2" fluid>
    <v-card color="basil">
      ...
      <div
        ...
        v-if="$apollo.queries.listThought.loading"
      >
        <v-progress-circular class="mt-10" :size="50" color="primary" indeterminate></v-progress-circular>
      </div>
...
export default {
  ...
  computed: {
    ...
    achieved() {
      return this.listThought.data.filter(item => item.isAchieved === true);
    },
    notAchieved: {
      get() {
        return this.listThought.data.filter(item => item.isAchieved === false || item.isAchieved === null);
      }
    }
  }
};
